...
if(isset($_POST['save_list'])){
   ...
   if($user_id != ''){
      ...
      $select_list = $conn->prepare("SELECT * FROM `bookmark2` WHERE user_id = ? AND playlist_id = ?");
      $select_list->execute([$user_id, $list_id]);

      if($select_list->rowCount() > 0){
         $remove_bookmark = $conn->prepare("DELETE FROM `bookmark2` WHERE user_id = ? AND playlist_id = ?");
         $remove_bookmark->execute([$user_id, $list_id]);
         $message[] = 'playlist removed!';
      }else{
         $insert_bookmark = $conn->prepare("INSERT INTO `bookmark2`(user_id, playlist_id) VALUES(?,?)");
...
<section class="playlist">
   ...
   <div class="row">
      ...
      <?php
         $select_playlist = $conn->prepare("SELECT * FROM `kursus_gratis` WHERE id = ? and status = ? LIMIT 1");
         ...
         if($select_playlist->rowCount() > 0){
            ...
            $select_bookmark = $conn->prepare("SELECT * FROM `bookmark2` WHERE user_id = ? AND playlist_id = ?");
...
<section class="videos-container">
   ...
   <div class="box-container">
      ...
      <a href="watch_video.php?get_id=<?= $fetch_content['id']; ?>" class="box">
         ...
         <h3 class="option-btn"><?= $fetch_content['link']; ?></h3>
      </a>
      <?php
            }
         }else{
