...
if(isset($_POST['save_list'])){
   ...
   if($user_id != ''){
      ...
      $select_list = $conn->prepare("SELECT * FROM `bookmark3` WHERE user_id = ? AND playlist_id = ?");
      $select_list->execute([$user_id, $list_id]);

      if($select_list->rowCount() > 0){
         $remove_bookmark = $conn->prepare("DELETE FROM `bookmark3` WHERE user_id = ? AND playlist_id = ?");
         $remove_bookmark->execute([$user_id, $list_id]);
         $message[] = 'playlist removed!';
      }else{
         $insert_bookmark = $conn->prepare("INSERT INTO `bookmark3`(user_id, playlist_id) VALUES(?,?)");
...
<section class="playlist">
   ...
   <div class="row">
      ...
      <?php
         $select_playlist = $conn->prepare("SELECT * FROM `kursus_bayar` WHERE id = ? and status = ? LIMIT 1");
         ...
         if($select_playlist->rowCount() > 0){
            ...
            $select_bookmark = $conn->prepare("SELECT * FROM `bookmark3` WHERE user_id = ? AND playlist_id = ?");
      ...
      <div class="col">
         ...
         <div class="details">
            ...
            <h3 class="option-btn"><?= $fetch_playlist['link']; ?></h3>
